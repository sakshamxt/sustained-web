...
  <div className={`flex items-end mb-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
    {/* Added max-w-full on mobile and adjusted on desktop */}
    <div className={`max-w-full md:max-w-md rounded-lg px-4 py-2 ${isUser ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'}`}>
      <p className="text-sm" style={{ overflowWrap: 'break-word' }}>{message}</p>
    </div>
  </div>
);
...
  return (
    ...
    // On desktop (md): inset-auto (resets full-screen), and positioned above the button
    <div className="fixed inset-0 z-40 flex flex-col bg-card animate-fade-in-up 
                    md:inset-auto md:bottom-28 md:right-8 md:h-[500px] md:w-[380px] 
