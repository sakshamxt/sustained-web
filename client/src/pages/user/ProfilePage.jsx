// src/pages/user/ProfilePage.jsx - FIXED FOR THEME CONSISTENCY
...
        <h1 className="text-4xl font-extrabold tracking-tight text-foreground md:text-5xl">Your Profile</h1>
        <p className="max-w-2xl mx-auto mt-4 text-lg text-muted-foreground">Manage your account details, track your progress, and view your achievements.</p>
      </header>
      
...
                    </Avatar>
                    {/* UPDATED: Using theme-aware colors */}
                    <h2 className="mt-4 text-2xl font-bold text-foreground">{user.username}</h2>
...
            </Card>
              {/* UPDATED: Using theme-aware colors */}
            <Card><CardHeader><CardTitle>Your Stats</CardTitle></CardHeader><CardContent className="space-y-6"><div className="flex items-start gap-4"><Star className="w-8 h-8 text-amber-500 mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">Points</p><p className="text-3xl font-bold text-primary">{user.points?.toLocaleString() ?? 0}</p></div></div><div className="flex items-start gap-4"><Zap className="w-8 h-8 text-red-500 mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">Current Streak</p><p className="text-3xl font-bold text-primary">{user.streak ?? 0} days</p></div></div></CardContent></Card>
...
        <main className="lg:col-span-2 space-y-8">
              {/* UPDATED: Using theme-aware colors */}
            <Card><CardHeader><CardTitle>Personal Information</CardTitle></CardHeader><CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="flex items-start gap-3"><Mail className="w-5 h-5 text-muted-foreground mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">Email</p><p className="text-foreground">{user.email}</p></div></div><div className="flex items-start gap-3"><User className="w-5 h-5 text-muted-foreground mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">Username</p><p className="text-foreground">{user.username}</p></div></div><div className="flex items-start gap-3"><MapPin className="w-5 h-5 text-muted-foreground mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">City</p><p className="text-foreground">{user.city || 'Not set'}</p></div></div><div className="flex items-start gap-3"><MapPin className="w-5 h-5 text-muted-foreground mt-1"/><div className="flex flex-col"><p className="text-sm text-muted-foreground">State</p><p className="text-foreground">{user.state || 'Not set'}</p></div></div></CardContent></Card>
            <Card>
...
                        <div className="flex justify-between items-center"><p className="font-semibold text-foreground">{course.sdgNumber}. {course.title}</p><p className="text-sm font-medium text-primary">{Math.round(course.progress)}%</p></div>
                        <Progress value={course.progress} />
                      </div>
                    ))
                  ) : (
                    // UPDATED: Using theme-aware colors
                    <p className="text-sm text-muted-foreground text-center py-4">You haven't enrolled in any courses yet. <Link to="/sdgs" className="text-primary hover:underline">Explore courses</Link> to get started!</p>
                  )}
                </CardContent>
            </Card>
        </main>
