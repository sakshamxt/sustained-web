// src/pages/sdg/SdgDetailPage.jsx - FINAL MODERN & FUNCTIONAL VERSION
...
  const [activityTextResponse, setActivityTextResponse] = useState("");
  ...
  const handleMarkAsComplete = async (contentType, contentId, submissionData = null) => {
    ...
    try {
      ...
      // Optimistically update user state for instant UI feedback
      const updatedUser = {
        ...user,
  ...
  const renderContentItem = (item, type) => {
    ...
    return (
      <div key={item._id} className="flex items-center justify-between p-3 transition-colors border rounded-md hover:bg-slate-50">
...
  if (!sdg) return <div className="py-10 text-center text-slate-500">SDG not found.</div>;
  
...
    <div className="container py-8 mx-auto">
      <header className="py-12 mb-12 text-center border-b">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 md:text-5xl">{sdg.title}</h1>
        <p className="max-w-2xl mx-auto mt-4 text-lg text-slate-600">{sdg.shortDescription}</p>
      </header>
...
        <main className="lg:col-span-2 space-y-8">
          <Card>
            ...
            <Tabs defaultValue="about" className="w-full">
              ...
              <TabsContent value="about" className="p-6 prose prose-lg max-w-none text-slate-600">
                {(Array.isArray(sdg.fullDescription) ? sdg.fullDescription : [sdg.fullDescription]).map((p, i) => <p key={i}>{p}</p>)}
              </TabsContent>
              <TabsContent value="lessons" className="p-6 space-y-2">
                {lessons.length > 0 ? lessons.map(l => renderContentItem(l, 'lesson')) : <p className="text-sm text-center text-slate-500 py-4">No lessons available for this course yet.</p>}
              </TabsContent>
              <TabsContent value="activities" className="p-6 space-y-2">
                 {activities.length > 0 ? activities.map(a => renderContentItem(a, 'activity')) : <p className="text-sm text-center text-slate-500 py-4">No activities available for this course yet.</p>}
              </TabsContent>
              <TabsContent value="presentations" className="p-6 space-y-2">
                {presentations.length > 0 ? presentations.map(p => renderContentItem(p, 'presentation')) : <p className="text-sm text-center text-slate-500 py-4">No presentations available for this course yet.</p>}
              </TabsContent>
            </Tabs>
          </Card>
        </main>
          ...
          <Card>
              ...
              <CardContent>
                <ul className="space-y-3">
                  {sdg.whatYouWillLearn.map((point, index) => <li key={index} className="flex items-start gap-3 text-sm text-slate-600"><CheckCircle className="w-4 h-4 text-primary flex-shrink-0 mt-0.5"/><span>{point}</span></li>)}
                </ul>
              </CardContent>
          </Card>
        </aside>
