// src/pages/sdg/SdgDetailPage.jsx - FIXED
...
import { CheckCircle, Info, BookOpen, ListChecks, Edit, Loader2, ExternalLink } from 'lucide-react'; // Added ExternalLink
...
    const requiresSubmission = type === 'activity' && item.submissionType === 'textResponse';
    ...
    // --- UPDATED LOGIC FOR PRESENTATIONS ---
    if (type === 'presentation') {
      return (
        <div key={item._id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 transition-colors border rounded-md hover:bg-secondary">
          <span className="text-sm font-medium text-foreground mb-2 sm:mb-0">{item.title}</span>
          <div className="flex items-center gap-2 self-end sm:self-center">
            <Button asChild size="sm" variant="outline" disabled={!isEnrolled}>
              <a href={item.urlOrContent} target="_blank" rel="noopener noreferrer">
...
    // Standard Lesson or Activity
    return (
      <div key={item._id} className="p-3 transition-colors border rounded-md">
...
              <TabsContent value="about" className="p-6 prose prose-lg max-w-none text-muted-foreground dark:prose-invert">
                {sdg.fullDescription && sdg.fullDescription.split('\n').map((paragraph, index) => (
...
                  {sdg.whatYouWillLearn && sdg.whatYouWillLearn.map((point, index) => <li key={index} className="flex items-start gap-3 text-sm text-muted-foreground"><CheckCircle className="w-4 h-4 text-primary flex-shrink-0 mt-0.5"/><span>{point}</span></li>)}
                 </ul>
              </CardContent>
          </Card>
        </aside>
